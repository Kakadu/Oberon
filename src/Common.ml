...
module Trap =
  struct
    ...
    let ulocate k   = H.find t (Obj.magic k)
    let locate  k   = try ulocate k with Not_found -> Locator.No
...
ostap (
  loc[item]: l:$ x:item r:$ {
    ...
    Trap.attach x (Msg.Locator.makeInterval loc r#loc); 
    x
  }
)
...
let apply f x = f x
let id    x   = x
let lift  x y = fun _ -> x y
let return x  = fun _ -> x
let (++) x y = fun ext s -> x (fun self -> y (fun _ -> ext self)) s
